...
const movimientosController = {
  createMovimiento: async (req, res) => {
    try {
      const { tipo, fecha, material_id, cantidad, responsable_id, motivo, responsable_id_recibe } = req.body;
      ...
      if (responsable_id_recibe) {
        const responsableRecibeResult = await stock.execute({
          sql: 'SELECT id FROM responsables WHERE id = ?',
          args: [responsable_id_recibe]
        });

        if (responsableRecibeResult.rows.length === 0) {
          return res.status(404).json({ message: 'Responsable (recibe) no encontrado' });
        }
      }
      ...
      const insertResult = await stock.execute({
        sql: `
    INSERT INTO movimientos (tipo, fecha, material_id, cantidad, responsable_id, motivo, responsable_id_recibe)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `,
        args: [
          tipo,
      ...
      res.status(201).json({
        ...
        movimiento: {
          ...
          responsable_id_recibe: responsable_id_recibe || null,
          motivo: motivo || null
        },
