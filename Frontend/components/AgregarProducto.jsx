...
const AgregarProducto = () => {
  ...
  */}
  ...
  import React, { useState } from "react";
  import { agregarProducto } from '../services/api';
// Función para agregar el producto (llama al backend)
...
const AgregarProductoForm = () => {
  ...
  const calcularPrecioConIva = () => {
    let precioNeto = parseFloat(formData.precio_neto) || 0;
    let precioConRecargo = precioNeto;

    // Aplica el recargo de 5% o 6.5% si se selecciona
    if (formData.recargo5) precioConRecargo *= 1.05;
    if (formData.recargo6_5) precioConRecargo *= 1.065;

    // Aplica la ganancia
    let precioConGanancia = precioConRecargo + (precioConRecargo * (parseFloat(formData.ganancia) || 0) / 100);

    // Aplica el IVA
    const iva = parseFloat(formData.iva) || 21;
    const precioFinal = precioConGanancia * (1 + iva / 100);

    return {
      precioConRecargo: precioConRecargo.toFixed(2),
      precioConGanancia: precioConGanancia.toFixed(2),
      precioConIva: precioFinal.toFixed(2),
    };
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const { precioConRecargo, precioConGanancia, precioConIva } = calcularPrecioConIva();

    const producto = {
      nombre: formData.nombre,
      precio_neto: formData.precio_neto,
      precio_con_iva: precioConIva,  // Precio con IVA ya calculado
      proveedor: formData.proveedor,
      modelo: formData.modelo,
    ...
    <div className="container mx-auto p-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* Campo Precio Neto */}
          <div>
            <label className="block text-sm font-medium text-gray-700">Precio Neto</label>
            <input
              type="text"
              name="precio_neto"
        ...
        <div className="space-y-2">
          <div>
            <label className="block text-sm font-medium text-gray-700">Recargos</label>
            <div className="flex space-x-4">
              <label>
                <input
                  type="checkbox"
                  name="recargo5"
                  checked={formData.recargo5}
                  onChange={handleChange}
                  className="mr-2"
                />
                5% de Recargo
        ...
        {/* Precios calculados */}
        <div className="mt-4">
          <p><strong>Precio con Recargo:</strong> {precioConRecargo}</p>
          <p><strong>Precio con Ganancia:</strong> {precioConGanancia}</p>
