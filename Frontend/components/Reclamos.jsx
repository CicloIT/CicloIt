...
import { obtenerReclamos, listaUsuarios } from "../services/api";
...
function Reclamos() {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const [reclamosData, usuariosData] = await Promise.all([
            obtenerReclamos(),
            listaUsuarios()
          ]);
  ...
  const reclamosFiltrados = reclamos?.filter(reclamo => {
    return (
      ...
      (!filtros.nombreCliente || reclamo.empresa.toLowerCase().includes(filtros.nombreCliente.toLowerCase()))
    );
  });
        ...
        {reclamosFiltrados.map((reclamo) => (
          ...
          >            
            <div className="grid grid-cols-2 gap-4">
              <p className="text-sm">
                <span className="font-semibold">Cliente:</span> {reclamo.empresa}
              </p>
              <p className="text-sm">
